...
import { getRecent4, prismaCount4 } from '@/components/getrecentchallenge';
...
export default function Home() {
	...
	const lines: Array<{ range: [number, number, number, number], label: string }> = [
		{ range: [33, 1, 36, 1], label: 'editableSection' }
	];

	useEffect(() => {
		(async () => {
			try {
				const content = await readReadmeFile(4);
			...
			<div className="w-3/5 h-full">
				...
				<Button color="warning"
					...
					onClick={async () => {
						...
						saveToFile(username, 4, editorRef.current?.getModel()?.getValue());
						const temp = await getRecent4(username, 0);
						setMessage(temp);
						setSkips(0);
					}}>Submit</Button>
			</div>
			<div className="w-2/5 h-full">
				...
				<div className="w-full h-1/8">
					<Button color="success" onClick={async () => {
						const size = await prismaCount4(username);
						const newSkips = skips + 1;
						setSkips(newSkips % size);
						const mes = await getRecent4(username, skips);
						setMessage(mes);
					}}>&lt;&lt;</Button>
					{skips}
					<Button color="success" onClick={async () => {
