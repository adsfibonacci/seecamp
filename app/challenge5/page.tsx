...
import { getRecent5, prismaCount5 } from '@/components/getrecentchallenge';
...
export default function Home() {
	...
	const lines: Array<{ range: [number, number, number, number], label: string }> = [
		{ range: [28, 1, 31, 1], label: 'editableSection' },
		{ range: [40, 1, 43, 1], label: 'editableSection' }
	];

	useEffect(() => {
		(async () => {
			try {
				const content = await readReadmeFile(5);
			...
			<div className="w-3/5 h-full">
				...
				<Button color="warning"
					...
					onClick={async () => {
						...
						saveToFile(username, 5, editorRef.current?.getModel()?.getValue());
						const temp = await getRecent5(username, 0);
						setMessage(temp);
						setSkips(0);
					}}>Submit</Button>
			</div>
			<div className="w-2/5 h-full">
				...
				<div className="w-full h-1/8">
					<Button color="success" onClick={async () => {
						const size = await prismaCount5(username);
						const newSkips = skips + 1;
						setSkips(newSkips % size);
						const mes = await getRecent5(username, skips);
						setMessage(mes);
					}}>&lt;&lt;</Button>
					{skips}
					<Button color="success" onClick={async () => {
